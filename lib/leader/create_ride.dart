...
class CreateRide extends StatefulWidget {
  ...
  @override
  _CreateRide createState() => new _CreateRide();
}
...
class _CreateRide extends State<CreateRide> {
  ...
  initState() {
    super.initState();
    formController.text = "${widget.user.displayName} - ${DateTime.now()}";
  }
  ...
  void submitRide(BuildContext context) {
    ...
    DocumentReference rideLeaderReference = Firestore.instance.collection('rideleaders').document(widget.user.uid);
    ...
    newRide.setData({'title': formController.text, 'date': DateTime.now(), 'leader': rideLeaderReference}).then((result){
      ...
      Navigator.of(context).push(
          MaterialPageRoute(
            // TODO Ride View
            builder: (context) => RideLeaderSection(user: widget.user, ride: newRide),
          )
      );
    }, onError: (e) {
      showDialog(
